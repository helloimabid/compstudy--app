...
export default function StatsScreen() {
  ...
  const loadStats = async () => {
    try {
      ...
      if (user) {
        ...
        setPersonalSessions(
          personalResponse.documents as unknown as StudySession[],
        );
      }
    } catch (error) {
  ...
  const processPersonalData = () => {
    const totalSeconds = personalSessions.reduce(
    ...
    const sessionsByDay = personalSessions.reduce((acc: any, session) => {
      ...
      const date = getLogicalDateLabel(dayResetHour, sessionDate, {
        weekday: "short",
        day: "numeric",
      });
    ...
    const totalDuration = Object.values(sessionsBySubject).reduce(
      (a: any, b: any) => a + b,
      0,
    ) as number;
    const subjectData = Object.keys(sessionsBySubject)
      .map((subject) => {
        const val = sessionsBySubject[subject];
        const pct =
          totalDuration > 0 ? ((val / totalDuration) * 100).toFixed(0) : "0";
        return {
          name: subject,
          value: val,
          percentage: pct,
  ...
  const { totalHours, dailyData, subjectData, totalDuration } =
    processPersonalData();
          ...
          <>
            ...
            <View style={styles.statsGrid}>
              <View style={[styles.statCard, styles.personalStatCard]}>
                <View
                  style={[styles.statIconContainer, styles.personalIconBox]}
                >
                      ...
                      <View style={styles.trendBarContainer}>
                        <LinearGradient
                          ...
                          style={[
                            ...
                            {
                              width:
                                `${Math.min((item.hours / 8) * 100, 100)}%` as any,
                            },
                          ]}
                        />
                      </View>
          ...
          <>
            ...
            <View style={styles.statsGrid}>
              ...
              <View style={styles.statCard}>
                <View
                  style={[styles.statIconContainer, styles.statIconSuccess]}
                >
              ...
              <View style={styles.statCard}>
                <View
                  style={[styles.statIconContainer, styles.statIconWarning]}
                >
                  <Zap size={20} color={Colors.dark.warning} />
                </View>
                <Text style={styles.statValue}>
                  {globalStats.activeStudents}
                </Text>
