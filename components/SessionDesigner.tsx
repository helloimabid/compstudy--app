                ...
                <View style={styles.modalContainer}>
                    ...
                    <View style={styles.tabContainer}>
                        <TouchableOpacity
                            ...
                            style={[styles.tabButton, viewMode === "view" && styles.tabButtonActive]}
                        >
                            <Text style={[styles.tabText, viewMode === "view" && styles.tabTextActive]}>
                        ...
                        {viewMode === "view" ? (
                            ...
                            <View style={styles.scheduleList}>
                                {!existingSchedule || existingSchedule.length === 0 ? (
                                    <View style={styles.emptyState}>
                                        <Text style={styles.emptyStateText}>No sessions scheduled yet</Text>
                                        <TouchableOpacity
                                            onPress={() => {
                                                setViewMode("create");
                                                setStartTime(getCurrentTime());
                                        ...
                                        >
                                            <Text style={styles.createFirstButtonText}>Create Your First Schedule</Text>
                                        </TouchableOpacity>
                                    </View>
                                ) : (
                                    existingSchedule.map((session, index) => (
                                        <View key={session.$id} style={styles.scheduleItemContainer}>
                                            {/* Timeline Connector */}
                                            {index > 0 && <View style={styles.timelineConnector} />}

                                            <View style={[
                                                styles.scheduleCard,
                                                session.type === "focus" ? styles.focusCard : styles.breakCard
                                            ]}>
                                                <View style={[
                                                    styles.iconContainer,
                                                    session.type === "focus" ? styles.focusIcon : styles.breakIcon
                                                ]}>
                                                    {session.type === "focus" ? <BookOpen size={16} color="#818cf8" /> : <Coffee size={16} color="#4ade80" />}
                                                </View>
                                                ...
                                                <View style={{ flex: 1 }}>
                                                    <View style={styles.scheduleMeta}>
                                                        <Text style={[
                                                            styles.typeTag,
                                                            session.type === "focus" ? { color: '#a5b4fc' } : { color: '#86efac' }
                                                        ]}>{session.type}</Text>
                                                        <Text style={styles.metaDivider}>•</Text>
                                                        <Text style={styles.metaText}>{Math.floor(session.duration / 60)} min</Text>
                                                        <Text style={styles.metaDivider}>•</Text>
                                                    ...
                                                    <Text style={styles.scheduleSubject}>
                                                        {session.subject || (session.type === "break" ? "Break" : "Focus Session")}
                                                    </Text>
                                                    ...
                                                    {session.goal && (
                                                        <View style={{ marginTop: 8 }}>
                                                            {(() => {
                                                                try {
                                                                    const goals: GoalItem[] = JSON.parse(session.goal);
                                                                    return goals.length > 0 ? (
                                                                        <View>
                                                                            {goals.map(g => (
                                                                                <View key={g.id} style={styles.miniGoalItem}>
                                                                                    <Square size={10} color="#818cf8" style={{ opacity: 0.5 }} />
                                                                                    <Text style={styles.miniGoalText}>{g.text}</Text>
                                                                                </View>
                                                                            ))}
                                                                        </View>
                                                                    ) : null;
                                                                } catch {
                                                                    return <Text style={styles.miniGoalText}>{session.goal}</Text>;
                                                                }
                                                            })()}
                                                        </View>
                                                    )}
                                                ...
                                                <View style={styles.actionsContainer}>
                                                    {onDeleteScheduledItem && (
                                                        <TouchableOpacity onPress={() => onDeleteScheduledItem(session.$id)} style={styles.iconButton}>
                                                    ...
                                                    {onStartScheduledSession && (
                                                        <TouchableOpacity
                                                            ...
                                                            style={[
                                                                styles.startButton,
                                                                session.type === "focus" ? { backgroundColor: Colors.dark.primary } : { backgroundColor: Colors.dark.success }
                                                            ]}
                                                        >
                            ...
                            <View style={styles.createContainer}>
                                ...
                                <View style={styles.timeInputContainer}>
                                    ...
                                    {showTimePicker && !DateTimePicker && (
                                        <View style={{ padding: 10, backgroundColor: 'rgba(255,0,0,0.1)', borderRadius: 8 }}>
                                            <Text style={{ color: '#ef4444', fontSize: 12 }}>
                                                Time picker native module missing. Please rebuild your dev client or enter time manually.
                                            </Text>
                                            <TextInput
                                                style={[styles.timePickerText, { borderBottomWidth: 1, borderBottomColor: Colors.dark.border, marginTop: 8 }]}
                                                value={startTime}
                                                onChangeText={setStartTime}
                                                placeholder="HH:MM"
                                                placeholderTextColor={Colors.dark.textMuted}
                                            />
                                            <TouchableOpacity onPress={() => setShowTimePicker(false)} style={{ marginTop: 8 }}>
                                ...
                                <View style={{ gap: 12 }}>
                                    {blocks.map((block, index) => (
                                        <View key={block.id} style={styles.scheduleItemContainer}>
                                            ...
                                            <View style={[
                                                styles.scheduleCard,
                                                block.type === "focus" ? styles.focusCard : styles.breakCard
                                            ]}>
                                                <View style={[
                                                    styles.iconContainer,
                                                    block.type === "focus" ? styles.focusIcon : styles.breakIcon
                                                ]}>
                                                    {block.type === "focus" ? <BookOpen size={16} color="#818cf8" /> : <Coffee size={16} color="#4ade80" />}
                                                </View>
                                                ...
                                                <View style={{ flex: 1 }}>
                                                    <View style={styles.editRow}>
                                                        ...
                                                        <View>
                                                            ...
                                                            <View style={styles.durationInputWrapper}>
                                                                <TextInput
                                                                    ...
                                                                    onChangeText={(text) => updateBlock(block.id, { duration: parseInt(text) || 0 })}
                                                                />
                                                        ...
                                                        {/* Action: Delete */}
                                                        <TouchableOpacity onPress={() => removeBlock(block.id)} style={styles.iconButton}>
                                                            <Trash2 size={16} color={Colors.dark.textMuted} />
                                                        </TouchableOpacity>
                                                    </View>
                                                    ...
                                                    {block.type === "focus" && (
                                                        <View style={{ marginTop: 12 }}>
                                                            ...
                                                            <TextInput
                                                                ...
                                                                placeholderTextColor={Colors.dark.textMuted}
                                                                value={block.subject}
                                                                onChangeText={(text) => updateBlock(block.id, { subject: text })}
                                                            />
                                                            ...
                                                            <View style={styles.goalsContainer}>
                                                                {block.goals?.map((goal) => (
                                                                    <View key={goal.id} style={styles.goalRow}>
                                                                        <Square size={14} color={Colors.dark.textMuted} />
                                                                        <TextInput
                                                                            style={styles.goalInput}
                                                                            value={goal.text}
                                                                            onChangeText={(text) => updateGoal(block.id, goal.id, text)}
                                                                            placeholder="Goal..."
                                                                            placeholderTextColor="#555"
                                                                        />
                                                                        <TouchableOpacity onPress={() => removeGoal(block.id, goal.id)}>
                                                                            <X size={14} color={Colors.dark.textMuted} />
                                                                        </TouchableOpacity>
                                                                    </View>
                                                                ))}
                                                                <TouchableOpacity onPress={() => addGoalToBlock(block.id)} style={styles.addGoalButton}>
                                                                    <Plus size={12} color={Colors.dark.textMuted} />
                                ...
                                <View style={styles.addButtonsContainer}>
                                    <TouchableOpacity
                                        style={[styles.addButton, { backgroundColor: 'rgba(99, 102, 241, 0.1)', borderColor: 'rgba(99, 102, 241, 0.2)' }]}
                                        onPress={() => addBlock("focus")}
                                    >
                                        <Plus size={16} color="#818cf8" />
                                        <Text style={[styles.addButtonText, { color: "#818cf8" }]}>Add Focus</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.addButton, { backgroundColor: 'rgba(34, 197, 94, 0.1)', borderColor: 'rgba(34, 197, 94, 0.2)' }]}
                                        onPress={() => addBlock("break")}
                                    >
                                        <Plus size={16} color="#4ade80" />
                                        <Text style={[styles.addButtonText, { color: "#4ade80" }]}>Add Break</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        )}
...
const styles = StyleSheet.create({
    ...
    timelineConnector: {
        position: 'absolute',
        top: -16,
        left: 28, // Center of icon (16 padding + 24/2 width/2 approx)
        width: 2,
        height: 16,
